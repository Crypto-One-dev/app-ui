...
export const correctChains = [
  ...
  {
    url: '/bridge',
    chains: [ChainMap.RINKEBY, ChainMap.BSC_TESTNET, ChainMap.FANTOM_TESTNET]
  }
]
  ...
  for (let i = 0; i < correctChains.length; i++) {
    if (path.includes(correctChains[i].url)) {
      if (correctChains[i].exact) {
        if (correctChains[i].url === path) return correctChains[i].chains
        else return [ChainMap.MAINNET]
      }
